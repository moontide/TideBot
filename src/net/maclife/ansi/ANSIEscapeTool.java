...
//System.out.println (s);
	...
	public static void PutsToScreenBuffer (List<CharBuffer> listVirtualTerminalBuffer, List<List<Map<String, Object>>>listLinesCharactersAttributes, StringBuffer sbSrc, Map<String, Object> currentAttribute)
	{
		...
		for (i=0; i<sbSrc.length (); i++)
		{
			...
			System.err.print (nRowNO + " 行已有属性数量 " + listLineCharactersAttributes.size () + " 个, iColumnIndex 索引=" + iColumnIndex + ", 输出字符=[" + String.format ("%c 0x%02X", ch, (int)ch) + "]. ");
	...
	public static List<String> ToIRCEscapeSequence (List<CharBuffer> listVirtualTerminalBuffer, List<List<Map<String, Object>>>listLinesCharactersAttributes)
	{
		...
		for (int iLine=0; iLine<listVirtualTerminalBuffer.size (); iLine++)
		{
			...
			for (int iColumn=0; iColumn<listLineCharactersAttributes.size (); iColumn++)
			{
				...
				if (l_reverse != r_reverse)	// bold 有变动，要输出 bold 属性 (irc 两次/偶数次 bold 后关闭 bold)
					sb.append (Colors.REVERSE);
				if (l_underline != r_underline)	// bold 有变动，要输出 bold 属性 (irc 两次/偶数次 bold 后关闭 bold)
				...
				{	// 背景色变化了
					if (r_256color_bg)
						sIRC_FG = XTERM_256_TO_IRC_16_COLORS [r_bg];
					else
						sIRC_FG = ANSI_16_TO_IRC_16_COLORS [r_bg-30][0];	// iBold -> 0 背景不高亮
				}

				if (l_fg != r_fg  && l_bg != r_bg)
				{	// 前景色、背景色都变化了
	...
	{
		return ConvertAnsiEscapeTo (sANSIString, 80);
	}
	...
	public static List<String> ConvertAnsiEscapeTo (String sANSIString, final int TERMINAL_COLUMNS)
	{
		...
		Map<String, Object> currentCharacterAttribute = new HashMap<String, Object> ();
/*
...
//HexDump (line);
		...
		while (matcher.find())
		{
			...
			{
				System.err.println ("光标在写入缓冲区前的位置=" + nCurrentRowNO + " 行 " + nCurrentColumnNO + " 列");
